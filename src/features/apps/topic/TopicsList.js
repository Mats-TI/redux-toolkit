...
import { Heading, Spinner, Checkbox, Flex } from "@chakra-ui/react";
...
export default function TopicsList({ getTopic, currentCategory }) {
	...
	return isLoading ? (
		...
		<>
			...
			<Row className="mt-2">
				<Col span={17}>
					<p className="small pl-3 text-muted">
						{topics.meta.totalCount[0].results} topics
					</p>
				</Col>
				<Col span={7}>
					<Switch onClick={info} size="small" defaultChecked />{" "}
					<span className="pl-1 small text-muted">Details</span>
				</Col>
			</Row>
			<Row className="mt-4">
				...
				{topics.data.map((topic, index) => (
					...
					>
						<Card
							bordered={true}
							hoverable="true"
							style={{ width: 300, marginTop: 0, marginRight: 20, padding: 0 }}
							className="slim-card p-0 ml-1 mt-2 m-0"
						>
							<Row className="p-0">
								<Col span={4} className="pt-3 pl-3">
									{selectCheck ? (
										<div
											style={{ display: selectCheck ? "block" : "none" }}
											className="mr-2"
										>
											<Checkbox></Checkbox>
										</div>
									) : (
										<span className="align-left muted">
											<Star size={15} />{" "}
										</span>
									)}
								</Col>
								<Col span={20} className="pr-5">
									<Title onClick={() => getTopic(topic.id)} level={5}>
										{topic.title}
									</Title>
